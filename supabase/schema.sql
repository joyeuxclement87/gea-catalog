...
-- Catalogue PDF status — single-row record for the one current catalogue PDF.
CREATE TABLE catalogue_pdf_status (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE UNIQUE INDEX idx_catalogue_pdf_status_singleton ON catalogue_pdf_status ((true));
